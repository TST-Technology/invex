...
import { CAPITAL_STRUCTURE_COLUMNS } from './Constants';
...
const CapitalStructure = ({ data, Loading }) => {
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      console.log(CAPITAL_STRUCTURE_COLUMNS.length);
      const finalData = CAPITAL_STRUCTURE_COLUMNS.map((columns) => {
        let newObj = {};
      ...
      setCapitalStructureData(finalData);
            ...
            <div className='table-responsive'>
              <table className='table table-bordered table-striped m-0 most_tables'>
                ...
                <tbody className='border-top-0'>
                  {capitalStructureData.map((row, index) => {
                    return (
                      <tr key={index}>
                        <td>
                          {row?.heading}{' '}
                        ...
                        {data &&
                          data.map((value, i) => {
                            return <td>{abbreviateNumber(row[`col${i}`])}</td>;
                          })}
                      </tr>
                    );
                  })}
